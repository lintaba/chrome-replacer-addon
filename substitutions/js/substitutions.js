...
chrome.runtime.sendMessage("config", function(response) {
  ...
  var substitute = (function() {
    ...
    for (i = replacements.length - 1; i >= 0; i--) {
      ...
      replacementsObject.push([original, replacements[i][1]+'XX__XX']);

      original = new RegExp("\\b" + replacements[i][1] + "\\b", "gi");
      replacementsObject.push([original, replacements[i][0]+'XX__XX']);
    }
    ...
    return function(node) {
      ...
      node.nodeValue = node.nodeValue.replace(/XX__XX/gi,'');
    };
  })();
